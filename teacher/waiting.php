...
$student_number = $user_profile_data['student_number'];
$unique_number = $student_number;
if ($component_name === 'ROTC') {
  
  $crossquery = "SELECT trainer_name FROM trainertable WHERE trainer_uniquenumber = '$unique_number'";
} elseif ($component_name === 'CWTS') {
  $unique_number = $student_number;
  $crossquery = "SELECT teacher_name FROM teachertable WHERE teacher_uniquenumber = '$unique_number'";
} else {
  // handle the case where $component_name is neither 'ROTC' nor 'CWTS'
  $crossquery = ""; // set $crossquery to an empty string to avoid "Query was empty" error
}

if (!empty($crossquery)) {
$crossquery_result = $con->query($crossquery);
$crossquery_data = $crossquery_result->fetch_assoc();
}
            ...
            <div class="insideform">
                <div class="minisidebar">
                    <div class="profilepic-container">
                        <a href="./teachergallery.php" class="picture-container">
                            <?php echo "<img class = 'profilepic' src='./$profileimage'><br>"; ?>
                            <i class="bi bi-camera-fill my-icon"></i>
                        </a>
                    </div>
                    <span><?php echo $crossquery_data["trainer_name"] ?? $crossquery_data["teacher_name"];?></span>
                </div>
                <div class="minicontent">
                    ...
                    <div class="inputContainer">
                      ...
                      <span><?php echo !empty($user_profile_data['group_name']) ? $user_profile_data['group_name'] : 'No Information'; ?></span>
                    </div>
                    <div class="inputContainer">
...
<script>
  ...
  $('#editPassword').off().click(function() {
    ...
    $('#passwordValue').replaceWith('<div class="passwordInputs"><input type="password" id="passwordInput" placeholder="New Password"><input type="password" id="repeatPasswordInput" placeholder="Confirm Password"></div>');
