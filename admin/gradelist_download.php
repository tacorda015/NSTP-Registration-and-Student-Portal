...
if (!isset($_GET['download'])) {
    ...
    $grade_query = "SELECT u.full_name, u.student_number, u.student_section, u.component_name, u.year_level, group_table.group_name, g.student_grade
    FROM useraccount u 
    ...
    $course_code_query = "SELECT course_code FROM coursetable WHERE course_name = '$course_name'";
    $course_code_result = $con->query($course_code_query);

    $course_code_data = $course_code_result->fetch_assoc();
    $course_code = $course_code_data['course_code'];
...
if ($grade_result && $grade_result->num_rows > 0) {
    while ($row = $grade_result->fetch_assoc()) {
        ...
        if($year_level == 'First Year'){
            ...
            $yearLevel = 'Invalid Year Level';
        }

        $full_section = $course_code . " " . $yearLevel . "0" . $semester_id . $section; 

        // Check if the worksheet for the section already exists
        if (!isset($sectionWorksheets[$full_section]) && $row['student_number'] !== null) {

            // $getSchedCode = "SELECT s.schedcode_number FROM schedcodetable s 
            // LEFT JOIN coursetable c ON s.department_id = c.department_id
            // WHERE s.student_section = '$section' AND s.schoolyear_id = $schoolyear_id AND s.semester_id = $semester_id AND s.course = '$course_name' AND s.year_level = $yearLevel";

            // $getSchedCode_result = $con->query($getSchedCode);
            // $getSchedCode_data = $getSchedCode_result->fetch_assoc();
            // if ($getSchedCode_result) {
            //     // Log the query and error to the PHP error log
            //     error_log("Error in SQL query: $getSchedCode - " . $con->error);
            // }
            ...
            if ($getSchedCode_result && $getSchedCode_result->num_rows > 0) {
                $getSchedCode_data = $getSchedCode_result->fetch_assoc();
                $schedcode_number = $getSchedCode_data['schedcode_number'];
            } else {
                // Handle the case where no results are found
                $schedcode_number = null;  // or any default value you want to assign
                // Optionally log a message to the error log indicating no results were found
                error_log("No results found for query: $getSchedCode");
            }
            ...
            $sheet->setCellValue('A12', 'Year and Section: ');
            $sheet->getStyle('A12')->getFont()->setBold(true); // Set bold font
            $objPHPExcel->getActiveSheet()->getStyle("A12")->getFont()->setSize(12);
            $sheet->setCellValue('B12', $full_section);
            $sheet->mergeCells('B12:E12');
            $sheet->getStyle('B12:E12')->applyFromArray($underlineStyle);
            $sheet->getStyle('B12:E12')->applyFromArray($alignmentLeft); // Set left alignment
            $objPHPExcel->getActiveSheet()->getStyle("B12")->getFont()->setSize(12);
            $sheet->getStyle('A12')->getAlignment()->setWrapText(false);

            $sheet->setCellValue('A14', 'Student Number');
            $sheet->setCellValue('B14', 'Student Name');
            $sheet->setCellValue('C14', 'Component Name');
            $sheet->setCellValue('D14', 'Group Name');
            $sheet->setCellValue('E14', 'Student Grade');
            ...
            $sectionWorksheets[$full_section] = $sectionIndex;
            $sectionRowCounter[$full_section] = 15;
        }

        // If the section has data, proceed to populate the rows
        if (isset($sectionWorksheets[$full_section])) {
            // Get the worksheet index and row counter for the current section
            $worksheetIndex = $sectionWorksheets[$full_section];
            $rowCounter = $sectionRowCounter[$full_section];
            ...
            $sectionRowCounter[$full_section] = $rowCounter;
        }
        ...
        $sheet->getStyle('A14:E' . ($rowCounter - 1))->applyFromArray($borderStyle);
