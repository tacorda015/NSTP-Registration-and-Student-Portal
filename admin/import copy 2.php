    ...
    if (isset($_FILES['import_file']) && $_FILES['import_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_ext, $allowed_ext)) {
            ...
            foreach ($data as $row) {
                ...
                $csvSurname = mysqli_real_escape_string($con, $row['A']);
                $csvFirstname = mysqli_real_escape_string($con, $row['B']);
                $csvmiddlename = mysqli_real_escape_string($con, $row['C']);
                $csvFull_name = mysqli_real_escape_string($con, $row['D']);
                $csvEmail_address = mysqli_real_escape_string($con, $row['E']);
                $csvContactNumber = mysqli_real_escape_string($con, $row['F']);
                $csvBaranggay = mysqli_real_escape_string($con, $row['G']);
                $csvCity = mysqli_real_escape_string($con, $row['H']);
                $csvProvince = mysqli_real_escape_string($con, $row['I']);
                $csvHomeAddress = mysqli_real_escape_string($con, $row['J']);
                $csvCourse = mysqli_real_escape_string($con, $row['K']);
                $csvYear_level = mysqli_real_escape_string($con, $row['L']);
                $csvStudent_section = mysqli_real_escape_string($con, $row['M']);
                $csvGender = mysqli_real_escape_string($con, $row['N']);
                $csvBirthday = mysqli_real_escape_string($con, $row['O']);
                $csvStudent_number = mysqli_real_escape_string($con, $row['P']);
                $csvComponent_name = mysqli_real_escape_string($con, $row['Q']);
                ...
                if($semester_id == 1){
                    ...
                    if (mysqli_num_rows($checkResult) === 0) {
                        ...
                        if (!$result) {
                            ...
                            $csvUseraccount_query = "INSERT INTO useraccount (serialNumber, password, role_account_id, surname, firstname, middlename, full_name, email_address, contactNumber, baranggay, city, province, homeaddress, course, year_level, student_section, gender, birthday, student_number, component_name, group_id, user_status, qrimage, picture, schoolyear_id, semester_id, sendEmail) VALUES (NULL, '$password', 2, '$csvSurname', '$csvFirstname', '$csvmiddlename', '$csvFull_name', '$csvEmail_address', '$csvContactNumber', '$csvBaranggay', '$csvCity', '$csvProvince', '$csvHomeAddress', '$csvCourse', '$csvYear_level', '$csvStudent_section', '$csvGender', '$csvBirthday', '$csvStudent_number', '$csvComponent_name', NULL, 'Active', '$qrfile', NULL, $schoolyear_id, $semester_id, 1)";
            ...
            $estimetedCwtsGroup = ceil($numberCwtsStudent / 50);
            
            if($estimetedCwtsGroup < 11){
                $estimetedCwtsGroup = 12;
            }else{
                $estimetedCwtsGroup = ceil($numberCwtsStudent / 50);
            }
            $estimetedRotcGroup = ceil($numberRotcStudent / 37);

            echo "<script>console.log('$numberCwtsGroup');</script>";
            echo "<script>console.log('$estimetedCwtsGroup');</script>";

            if($numberCwtsGroup <= $estimetedCwtsGroup){
                for ($i = $numberCwtsGroup; $i < $estimetedCwtsGroup; $i++) {
                    $group_name = $cwts_groups[$i];
                    $insert_query_cwts = "INSERT INTO grouptable (group_name, incharge_person, component_id, number_student, schoolyear_id, semester_id, date_created, date_updated) VALUES ('$group_name', NULL, 2, 50, $schoolyear_id, $semester_id, NOW(), NOW())";
            ...
            if($studentNoGroupCWTSResult->num_rows > 0){
                while ($row = $studentNoGroupCWTSResult->fetch_assoc()) {
                    ...
                    $placingStudentCWTSGroup = "SELECT * FROM grouptable WHERE component_id = 2 AND schoolyear_id = $schoolyear_id AND semester_id = $semester_id";
            ...
            $studentNoGroupROTCQuery = "SELECT * FROM useraccount WHERE role_account_id = 2 AND component_name = 'ROTC' AND schoolyear_id = $schoolyear_id AND semester_id = $semester_id ORDER BY gender, student_section";
            ...
            if($studentNoGroupROTCResult->num_rows > 0){
                while ($row = $studentNoGroupROTCResult->fetch_assoc()) {
                    ...
                    $placingStudentROTCGroup = "SELECT * FROM grouptable WHERE component_id = 1 AND schoolyear_id = $schoolyear_id AND semester_id = $semester_id";
            ...
            <div class="buttonsContainer">
                <div class="buttonHolder">
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#uploadmodal">
                        <i class='bx bx-cloud-upload' ></i>Upload Enrolled Student
                    </button>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addstudentmodal">
                        <i class='bx bx-user-plus'></i>Add Student Record
                    </button>
                </div>
            </div>
              ...
              <div class="modal-dialog">
                ...
                if($group_check_result){
                    ...
                    if ($enrollment_algo_result) {
                        ...
                        $cwts_number_of_student = 50; // 50 is the number of student each group
                ...
                $query .= " ORDER BY enrolledstudent_id DESC LIMIT $recordsPerPage OFFSET $offset";
